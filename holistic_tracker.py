...
class HolisticTracker(Tracker):
    def __init__(self) -> None:
        ...
        self.out_queues = {}
        self.threads = {}
        self.reprojection_errors = {}  # 存儲每個攝像頭的重投影誤差
    ...
    def set_camera_params(self, port: int, camera_params: dict):
        """設置相機參數，用於計算重投影誤差"""
        self.reprojection_errors[port] = ReprojectionError(camera_params)

    def calculate_reprojection_error(self, port: int, points_3d: np.ndarray, points_2d: np.ndarray) -> Tuple[float, np.ndarray]:
        """計算指定攝像頭的重投影誤差"""
        if port not in self.reprojection_errors:
            raise ValueError(f"Camera parameters not set for port {port}")
        return self.reprojection_errors[port].calculate_reprojection_error(points_3d, points_2d)

    def visualize_reprojection(self, port: int, frame: np.ndarray, points_2d: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
        """在圖像上可視化重投影誤差"""
        if port not in self.reprojection_errors:
            raise ValueError(f"Camera parameters not set for port {port}")
        mean_error, reprojected_points = self.calculate_reprojection_error(port, points_3d, points_2d)
        
        # 在圖像上添加重投影誤差信息
        frame = self.reprojection_errors[port].visualize_reprojection(frame, points_2d, reprojected_points)
        cv2.putText(frame, f"Reprojection Error: {mean_error:.2f}px", 
