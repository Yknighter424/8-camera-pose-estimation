...
def get_aruco_axis(img_l1, img_l2, img_l3, img_c, img_r1, img_r2, img_r3, img_r4, 
    ...
    # 檢查是否使用舊版API
    is_old_api = False
    if isinstance(aruco_detector, dict) and 'is_old_api' in aruco_detector:
        is_old_api = True
        aruco_dict = aruco_detector['dict']
        aruco_params = aruco_detector['params']
    
    # 在八個相機圖像中檢測ArUco標記
    if is_old_api:
        # 使用舊版API
        corners_l1, ids_l1, _ = cv2.aruco.detectMarkers(img_l1, aruco_dict, parameters=aruco_params)
        corners_l2, ids_l2, _ = cv2.aruco.detectMarkers(img_l2, aruco_dict, parameters=aruco_params)
        corners_l3, ids_l3, _ = cv2.aruco.detectMarkers(img_l3, aruco_dict, parameters=aruco_params)
        corners_c, ids_c, _ = cv2.aruco.detectMarkers(img_c, aruco_dict, parameters=aruco_params)
        corners_r1, ids_r1, _ = cv2.aruco.detectMarkers(img_r1, aruco_dict, parameters=aruco_params)
        corners_r2, ids_r2, _ = cv2.aruco.detectMarkers(img_r2, aruco_dict, parameters=aruco_params)
        corners_r3, ids_r3, _ = cv2.aruco.detectMarkers(img_r3, aruco_dict, parameters=aruco_params)
        corners_r4, ids_r4, _ = cv2.aruco.detectMarkers(img_r4, aruco_dict, parameters=aruco_params)
    else:
        # 使用新版API
...
def triangulate_points_multi_cameras(poses, camera_params, projection_matrices, visibilities):
    ...
    # 定義重投影誤差計算函數（用於非線性優化）
    def reprojection_error(point_3d, camera_list, point_2d_list, visibility_list, cam_P_list, cam_param_list):
        """
        計算給定3D點的重投影誤差
        Args:
            point_3d: 待優化的3D點坐標 [3,]
            camera_list: 相機ID列表
            point_2d_list: 對應的2D觀測點列表
            visibility_list: 對應的可見度列表
            cam_P_list: 對應的投影矩陣列表
            cam_param_list: 對應的相機參數列表
        Returns:
            weighted_error: 加權重投影誤差
        """
        total_error = 0
        total_weight = 0
        
        for i, cam_id in enumerate(camera_list):
            mtx, dist = cam_param_list[i]
            P = cam_P_list[i]
            visibility = visibility_list[i]
            
            # 權重低於閾值的點給予較小權重但不完全排除
            if visibility < 0.5:
                weight = 0.1
            else:
                weight = visibility
            ...
            # 投影3D點到2D
            point_3d_homo = np.append(point_3d, 1.0)  # 齊次坐標
            point_2d_proj_homo = P @ point_3d_homo
            point_2d_proj = point_2d_proj_homo[:2] / point_2d_proj_homo[2]
            
            # 應用相機畸變
            point_3d_cam = P[:, :3] @ point_3d + P[:, 3]
            point_3d_cam = point_3d_cam / point_3d_cam[2]
            
            # 正規化坐標
            x_norm = point_3d_cam[0]
            y_norm = point_3d_cam[1]
            
            # 計算畸變
            r2 = x_norm*x_norm + y_norm*y_norm
            radial_distortion = 1 + dist[0]*r2 + dist[1]*r2*r2 + dist[4]*r2*r2*r2
            tx = 2*dist[2]*x_norm*y_norm + dist[3]*(r2 + 2*x_norm*x_norm)
            ty = dist[2]*(r2 + 2*y_norm*y_norm) + 2*dist[3]*x_norm*y_norm
            
            # 添加畸變
            x_dist = x_norm * radial_distortion + tx
            y_dist = y_norm * radial_distortion + ty
            
            # 轉換到像素坐標
            point_2d_distorted = np.array([
                mtx[0, 0]*x_dist + mtx[0, 2],
                mtx[1, 1]*y_dist + mtx[1, 2]
            ])
            
            # 計算與觀測點的誤差
            error = np.linalg.norm(point_2d_list[i] - point_2d_distorted)
    ...
    for pt_idx in range(num_points):
        ...
        A = []
        weights = []
        valid_cameras = []  # 記錄參與重建的相機
        valid_pts2d = []    # 記錄有效的2D點
        valid_weights = []  # 記錄有效的權重
        valid_params = []   # 記錄有效的相機參數
        valid_Ps = []       # 記錄有效的投影矩陣
        ...
        for cam_id, pose in poses.items():
            ...
            # 降低可見度閾值，允許更多的點參與計算，但給予不同的權重
            if visibility < 0.5:  # 閾值從0.98降低到0.5
                continue
                
            valid_cameras.append(cam_id)  # 記錄有效相機
            valid_pts2d.append(pose[pt_idx])  # 記錄有效2D點
            valid_weights.append(visibility)  # 記錄有效權重
            valid_params.append((mtx, dist))  # 記錄有效相機參數
            valid_Ps.append(P)  # 記錄有效投影矩陣
        ...
        point_4d = Vt[-1]
        point_3d_init = (point_4d / point_4d[3])[:3]
        
        # 第二步：使用非線性優化改進解
        if len(valid_cameras) >= 2:  # 至少需要兩個相機才能進行非線性優化
            try:
                # 使用scipy.optimize.minimize進行非線性優化
                result = minimize(
                    reprojection_error,
                    point_3d_init,  # DLT结果作为初值
                    args=(valid_cameras, valid_pts2d, valid_weights, valid_Ps, valid_params),
                    method='Powell',  # 使用Powell优化方法
                    options={'disp': False, 'maxiter': 1000}
                )
                
                if result.success:
                    point_3d_final = result.x
                    # 計算優化前後的誤差改進
                    error_init = reprojection_error(point_3d_init, valid_cameras, valid_pts2d, valid_weights, valid_Ps, valid_params)
                    error_final = reprojection_error(point_3d_final, valid_cameras, valid_pts2d, valid_weights, valid_Ps, valid_params)
                    
                    # 如果優化顯著改進了誤差，使用優化後的解
                    if error_final < error_init * 0.9:  # 至少10%的改進
                        points_3d.append(point_3d_final)
                        print(f"Point {pt_idx}: Optimization improved error from {error_init:.2f} to {error_final:.2f}")
                    else:
                        points_3d.append(point_3d_init)
                        print(f"Point {pt_idx}: Optimization did not significantly improve error ({error_init:.2f} vs {error_final:.2f})")
                else:
                    points_3d.append(point_3d_init)
                    print(f"Point {pt_idx}: Optimization failed, using linear solution")
            except Exception as e:
                points_3d.append(point_3d_init)
                print(f"Point {pt_idx}: Optimization error: {e}, using linear solution")
        else:
            points_3d.append(point_3d_init)
...
def setup_aruco_detector():
    ...
    try:
        ...
        print(f"尝试使用新版API失败: {e}")
        # 尝试使用旧版API
        try:
            aruco_dict = cv2.aruco.Dictionary_get(cv2.aruco.DICT_ARUCO_ORIGINAL)
            aruco_params = cv2.aruco.DetectorParameters_create()
            # 旧版API没有ArucoDetector类，返回词典和参数
            return {"dict": aruco_dict, "params": aruco_params, "is_old_api": True}
        except Exception as e2:
            print(f"尝试使用旧版API也失败: {e2}")
            raise RuntimeError("无法初始化ArUco检测器，请检查OpenCV版本")
...
def visualize_reprojection_errors(error_data):
    ...
    plt.title('reprojection error distribution of each camera')
